...
import {DataSourceApi, DataSourceInstanceSettings, DataQueryRequest, DataQueryResponse,LoadingState } from '@grafana/data';
...
export class DataSource extends DataSourceApi<APMDataQuery, DXAPMDataSourceOptions> {
  ...
  async query(
    options: DataQueryRequest<APMDataQuery>
  ...
  async queryTable(target, from, to, frequency) {
    ...
    return getBackendSrv().datasourceRequest(requestPayload).then((response) => {
  ...
  processSeries(result, seriesList, target) {
    ...
    result.columns.map((column,j) => {
      switch(column.name) {
        case "agent_host": metricFullPath[2] = j;
        break;
        case "agent_process":  metricFullPath[3] = j;
        break;
        case "agent_name": metricFullPath[4] = j;
        break;
        case "domain_name": metricFullPath[1] = j;
        break;
        case "metric_path": metricFullPath[5] = j;
        break;
        case "metric_attribute": metricFullPath[6] = j;
        break;
        case "agg_value" : metricFullPath[7] = j;
        break;
        case "ts": metricFullPath[8] = j;
        break;
        default: let columnName = column.name;
        if(columnName.includes('agg') && isNull(metricFullPath[7]))
        {
          metricFullPath[7] = j;
          break;
        }
    ...
    result.rows.map((row) => {
      ...
      seriesObj.datapoints.push(
    [row[metricFullPath[7]], row[metricFullPath[8]]]);
